...
import { getMockUser, getRolePath, isMockAuthEnabled } from "@/lib/mock-auth";
    ...
    useEffect(() => {
        const checkRoleAndRedirect = async () => {
            if (isMockAuthEnabled()) {
                ...
                if (mock) {
                    router.push(getRolePath(mock.role));
                } else {
            ...
            if (res.success && res.data) {
                const { user } = res.data as { user: { roles?: Array<{ role: { name: string } }> } };
