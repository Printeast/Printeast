...
export function CustomizableProducts() {
    const t = useTranslations('CustomizableProducts');
    const containerRef = useRef<HTMLDivElement>(null)

    const PRODUCT_ITEMS = useMemo(() => [
        {
            id: "1",
            title: t('items.apparel.title'),
            description: t('items.apparel.description'),
            src: "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?q=80&w=800&auto=format&fit=crop",
            className: "bg-[#F3F4F6] text-gray-900",
        },
        {
            id: "2",
            title: t('items.drinkware.title'),
            description: t('items.drinkware.description'),
            src: "https://images.unsplash.com/photo-1514228742587-6b1558fcca3d?q=80&w=800&auto=format&fit=crop",
            className: "bg-[#0B3B44] text-white",
        },
        {
            id: "3",
    ...
    return (
        ...
        >
            <div className="sticky top-0 h-screen overflow-hidden flex flex-col justify-center py-10">
                ...
                <div className="relative w-full pl-6 lg:pl-16 py-4">
                    <motion.div
                        style={{ x }}
                        className="flex gap-8 w-max px-4 will-change-transform"
                    >
                        {PRODUCT_ITEMS.map((item, index) => (
                            <ProductCard key={item.id} item={item} index={index} exploreText={t('explore')} />
                        ))}
                    </motion.div>
                </div>
            </div>
...
const ProductCard = memo(({ item, index, exploreText }: { item: any, index: number, exploreText: string }) => {
    return (
        <motion.div
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.4, delay: index * 0.05 }}
            className={cn(
                "group relative w-[400px] h-[380px] lg:w-[450px] lg:h-[440px] flex-shrink-0 rounded-md p-8 flex flex-col justify-between overflow-hidden transition-all duration-500 transform-gpu",
                item.className
            )}
        >
            {/* Header Content */}
            <div className="relative z-10 flex flex-col gap-4">
                <div className="flex justify-between items-start">
                    <h3 className="text-4xl lg:text-5xl font-black tracking-tight leading-tight">{item.title}</h3>
                    <span className="text-sm font-bold opacity-50 border border-current px-3 py-1 rounded-full whitespace-nowrap">
                        0{index + 1}
                    </span>
                </div>
                <p className="text-xl font-bold opacity-70 leading-relaxed max-w-[90%] tracking-tight">
                    {item.description}
                </p>
            </div>
            ...
            <div className="relative w-full h-[62%] mt-auto group-hover:scale-[1.02] transition-transform duration-700 cubic-bezier(0.25, 1, 0.5, 1)">
                <div className="absolute inset-0 rounded-md overflow-hidden bg-black/5">
                    <Image
                        src={item.src}
                        alt={item.title}
                        fill
                ...
                <div className="absolute bottom-5 right-5 bg-white/30 backdrop-blur-md text-current px-5 py-2.5 rounded-md text-sm font-bold opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500 delay-75">
                    {exploreText}
                </div>
            </div>
        </motion.div>
